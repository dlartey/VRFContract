const Lottery = artifacts.require("VRFv2Consumer10");
const ETHER = 10**18;
contract('VRFv2Consumer10', (accounts) => {
  ...
  it('randomResult should be 0', async () => {
    const lotteryInstance = await Lottery.deployed();
    const randomResult = await lotteryInstance.randomResult.call();

    assert.equal(randomResult, 0, "randomResult should be 0");
  });

  // Test that lotteryID should be 1 initially
  it('lotteryId shoule be 0', async () => {
    const lotteryInstance = await Lottery.deployed();
    const lotteryId = await lotteryInstance.lotteryId.call();

    assert.equal(lotteryId, 0, "lotteryId shoule be 0");
  });

  // Test that lastRequestId should be 0 initially
  it('lastRequestId should be 0', async () => {
    const lotteryInstance = await Lottery.deployed();
    const lastRequestId = await lotteryInstance.lastRequestId.call();

    assert.equal(lastRequestId, 0, "lastRequestId should be 0");
  });
